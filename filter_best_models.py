...
def filter_overfitting_analysis():
    ...
    fma_final = fma_sorted[['Rank', 'model', 'Training Acc (%)', 'Validation Acc (%)', 'Overfitting Gap (%)']]
    gtzan_final = gtzan_sorted[['Rank', 'model', 'Training Acc (%)', 'Validation Acc (%)', 'Overfitting Gap (%)']]
    
    # Rename columns
    fma_final.columns = ['Rank', 'Model', 'Training Acc (%)', 'Validation Acc (%)', 'Overfitting Gap (%)']
    gtzan_final.columns = ['Rank', 'Model', 'Training Acc (%)', 'Validation Acc (%)', 'Overfitting Gap (%)']
...
def main():
    ...
    print(f"  Range: {fma_table['Overfitting Gap (%)'].min():.1f}% to {fma_table['Overfitting Gap (%)'].max():.1f}%")
    print(f"\nGTZAN Dataset:")
    print(f"  Models: {len(gtzan_table)}")
    print(f"  Avg Overfitting: {gtzan_table['Overfitting Gap (%)'].mean():.1f}%")
    print(f"  Range: {gtzan_table['Overfitting Gap (%)'].min():.1f}% to {gtzan_table['Overfitting Gap (%)'].max():.1f}%")
