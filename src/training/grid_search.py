...
class GridSearchTrainer:
    ...
    def _train_single_combination(
        ...
        **kwargs,
        ...
        # Get final metrics
        final_train_loss = (
            training_history["train_losses"][-1]
            if training_history["train_losses"]
            else float("inf")
        )
        final_val_loss = (
            training_history["val_losses"][-1]
            if training_history["val_losses"]
            else float("inf")
        )
        final_train_acc = (
            training_history["train_accuracies"][-1]
            if training_history["train_accuracies"]
            else 0.0
        )
        final_val_acc = (
            training_history["val_accuracies"][-1]
        ...
        best_val_loss = (
            min(training_history["val_losses"])
            if training_history["val_losses"]
            else float("inf")
        )
        best_val_acc = (
            max(training_history["val_accuracies"])
            if training_history["val_accuracies"]
            else 0.0
        )
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        best_val_acc_idx = np.argmax(
            [r.get("best_val_acc", 0) for r in successful_runs]
        )
        best_val_loss_idx = np.argmin(
            [r.get("best_val_loss", float("inf")) for r in successful_runs]
        )
