...
class ModelEvaluator:
    ...
    def __init__(self, model: BaseModel, device: Optional[str] = None, logger: Optional[logging.Logger] = None):
        self.model = model
    ...
    def generate_evaluation_plots(
        ...
        if 'ks_stats' in results:
            self._plot_ks_curves(
                results,
                class_names,
                os.path.join(output_dir, 'ks_curves.png')
            )
    ...
    def _plot_roc_curves(
        ...
        plt.text(0.5, 0.5, f"ROC AUC: {results['roc_auc']:.4f}", 
    ...
    def _get_predictions_from_results(self, results: Dict[str, Any]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    def _plot_precision_recall_curves(
        ...
        if 'binary' in precision_recall:
            ...
            for class_idx, pr_data in precision_recall['multiclass'].items():
                class_name = class_names[int(class_idx.split('_')[1])] if class_names else class_idx
                plt.plot(pr_data['recall'], pr_data['precision'], 
