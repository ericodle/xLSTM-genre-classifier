...
from core.constants import (
    DEFAULT_TRAIN_SIZE, DEFAULT_VAL_SIZE, DEFAULT_TEST_SIZE,
    DEFAULT_FIGURE_WIDTH, DEFAULT_FIGURE_HEIGHT, DEFAULT_DPI
)
...
class ModelTrainer:
    ...
    def _load_csv_data(self, data_path: str):
        ...
        if len(valid_classes) < len(label_counts):
            ...
            self.logger.info(f"Filtered to {len(valid_classes)} classes with at least {min_samples_per_class} samples each")
            self.logger.info(f"Removed {len(label_counts) - len(valid_classes)} classes with insufficient samples")
        
        # Preprocess data
        self._preprocess_data()
        
        self.logger.info(f"Loaded {len(self.features)} samples with {len(np.unique(self.labels))} classes")
    ...
    def _load_json_data(self, data_path: str):
        ...
        if 'features' in data and 'labels' in data:
            ...
            self.logger.info(f"MFCC features: {len(features_list)} samples, frame range: {min_frames}-{max_frames}")
    ...
    def _preprocess_data(self):
        ...
        (self.X_train, self.X_val, self.X_test, 
         self.y_train, self.y_val, self.y_test) = self.preprocessor.split_data(
            ...
            train_size=DEFAULT_TRAIN_SIZE, val_size=DEFAULT_VAL_SIZE, test_size=DEFAULT_TEST_SIZE
        )
