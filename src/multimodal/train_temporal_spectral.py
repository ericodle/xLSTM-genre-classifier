#!/usr/bin/env python3
"""
Training script for simplified temporal + spectral multimodal model.
"""
...
from multimodal.temporal_spectral_model import TemporalSpectralModel
...
class TemporalSpectralTrainer:
    """Trainer for temporal+spectral model."""
    
    def __init__(self, model: TemporalSpectralModel, device: torch.device, class_names: List[str]):
        self.model = model
        self.device = device
    ...
    def train_epoch(self, train_loader: DataLoader, optimizer: optim.Optimizer, 
        ...
        for batch_idx, (features, labels) in enumerate(train_loader):
            ...
            for feature in features:
                feature = feature.to(self.device) if hasattr(feature, 'to') else feature
                output = self.model(feature)
                batch_outputs.append(output)
            
            # Stack outputs
            outputs = torch.stack(batch_outputs).squeeze(1) if len(batch_outputs) > 1 else batch_outputs[0]
...
def load_multimodal_data(data_path: str, feature_extractor: MultimodalFeatureExtractor) -> Tuple[List[MultimodalFeatures], List[str]]:
    ...
    for i, (features, label) in enumerate(zip(data['features'], data['labels'])):
        ...
        multimodal_features = MultimodalFeatures(
            ...
            # Statistical features (not used, but required for MultimodalFeatures)
...
def main():
    parser = argparse.ArgumentParser(description='Train temporal+spectral multimodal model')
    ...
    parser.add_argument('--fusion', type=str, default='learned', choices=['weighted', 'learned'],
                       help='Fusion method')
    ...
    # Create trainer
    trainer = TemporalSpectralTrainer(model, device, class_names)
