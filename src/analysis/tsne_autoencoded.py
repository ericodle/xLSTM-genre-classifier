...
def analyze_feature_statistics(features, labels):
    ...
    for genre in unique_genres:
        ...
        print(
            f"{genre:12}: {np.sum(genre_mask):3d} songs, "
...
def create_tsne_plots(features_2d, labels, mapping, output_dir):
    ...
    for i, genre in enumerate(unique_genres):
        mask = labels == genre
        plt.scatter(
            features_2d[mask, 0], features_2d[mask, 1], c=[colors[i]], label=genre, alpha=0.7, s=50
        )
    ...
    for i, genre in enumerate(unique_genres):
        ...
        mask = labels == genre
        ax.scatter(features_2d[mask, 0], features_2d[mask, 1], c=colors[i], alpha=0.7, s=50)
...
def main():
    ...
    parser.add_argument("--input", "-i", required=True, help="Path to autoencoded JSON file")
    parser.add_argument(
        "--output-dir", "-o", default="outputs/tsne_analysis", help="Output directory for plots"
    )
    parser.add_argument(
        "--perplexity", "-p", type=int, default=30, help="t-SNE perplexity parameter"
    )
    parser.add_argument(
        "--iterations", "-n", type=int, default=1000, help="Number of t-SNE iterations"
    )
    ...
    try:
        ...
        features_2d, encoded_labels = run_tsne_analysis(
            features, labels, mapping, args.output_dir, args.perplexity, args.iterations
        )
