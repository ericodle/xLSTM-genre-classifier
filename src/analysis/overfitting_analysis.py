...
def create_final_formatted_table(analysis_df):
    ...
    for model, dataset in model_dataset_order:
        ...
        model_row = analysis_df[(analysis_df['model'] == model) & (analysis_df['dataset'] == dataset)]
...
def main():
    ...
    if len(overfitting_gaps) > 0:
        print(f"Overfitting gap range: {overfitting_gaps.min():.1f}% to {overfitting_gaps.max():.1f}%")
