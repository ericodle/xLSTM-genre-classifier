...
def create_comparison_plots(results, mapping, output_dir, feature_dim):
    ...
    plt.bar(
        ...
        width,
        yerr=orig_stds,
        label=f"Original {feature_dim}D MFCC",
        alpha=0.8,
        capsize=5,
    )
    plt.bar(
        x + width / 2, tsne_means, width, yerr=tsne_stds, label="2D t-SNE", alpha=0.8, capsize=5
    )
...
def main():
    ...
    parser.add_argument("--input", "-i", required=True, help="Path to MFCC JSON file")
    parser.add_argument("--tsne-coords", "-t", help="Path to saved t-SNE coordinates (optional)")
    parser.add_argument(
        "--output-dir",
        "-o",
        default="outputs/classification_test",
