...
class DataManager:
    ...
    def load_json_data(self, json_path: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(features)} samples with {len(mapping)} classes")
    ...
    def get_class_distribution(self, labels: np.ndarray, mapping: List[str]) -> Dict[str, int]:
