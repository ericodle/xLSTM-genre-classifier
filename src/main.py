...
from core.constants import EXIT_SUCCESS, EXIT_FAILURE, EXIT_INTERRUPT, EXIT_FILE_NOT_FOUND, EXIT_EVALUATION_FAILED
from core.data_loader import DataManager
from data.mfcc_extractor import MFCCExtractor
from data.dataset_agnostic_mfcc_extractor import DatasetAgnosticMFCCExtractor, DatasetFactory
...
Examples:
  ...
  python main.py train --config config.json --data data.json --model CNN --output output/
    ...
    # Extract command
    extract_parser = subparsers.add_parser('extract', help='Extract MFCC features from audio files')
    extract_parser.add_argument('--input', required=True, help='Input music directory path')
    extract_parser.add_argument('--output', required=True, help='Output directory path')
    extract_parser.add_argument('--name', required=True, help='Output filename (without extension)')
    extract_parser.add_argument('--dataset-type', choices=['auto', 'gtzan', 'fma'], default='auto', 
                               help='Dataset type (auto-detected if not specified)')
    extract_parser.add_argument('--fma-api-key', help='FMA API key (or set FMA_API_KEY environment variable)')
...
"""
    ...
    eval_parser.add_argument('--model', required=True, help='Path to trained model file')
    eval_parser.add_argument('--data', required=True, help='Path to MFCC features JSON file')
    eval_parser.add_argument('--output', required=True, help='Output directory for evaluation results')
    eval_parser.add_argument('--config', help='Path to configuration file')
    
    # Train command
    train_parser = subparsers.add_parser('train', help='Train a model')
    train_parser.add_argument('--data', required=True, help='Path to MFCC features JSON file')
...
def extract_features(input_path: str, output_path: str, name: str, 
                    dataset_type: Optional[str] = None, fma_api_key: Optional[str] = None,
                    fma_tracks_csv: Optional[str] = None, n_mfcc: int = 13,
                    config_path: Optional[str] = None, logger=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if dataset_type is None or dataset_type == 'auto':
            ...
            dataset = DatasetFactory.create_dataset(input_path, 'auto', tracks_csv=fma_tracks_csv)
        else:
            ...
            if dataset_type.lower() == 'gtzan':
                ...
                if not fma_api_key and 'FMA_API_KEY' not in os.environ:
                    raise ValueError("FMA API key is required for FMA dataset. Use --fma-api-key or set FMA_API_KEY environment variable.")
                dataset = FMADataset(input_path, fma_api_key or os.getenv('FMA_API_KEY'), fma_tracks_csv)
            else:
        ...
        if logger:
            ...
            logger.info(f"Features extracted: {result['metadata']['total_samples']} samples")
    ...
    try:
        ...
        if data_path.lower().endswith('.csv'):
            ...
            if 'label' in df.columns:
                ...
                valid_classes = [label for label, count in label_counts.items() if count >= min_samples_per_class]
                ...
                if len(valid_classes) < len(label_counts):
                    ...
                    logger.info(f"Filtered to {len(valid_classes)} classes with at least {min_samples_per_class} samples each")
                    logger.info(f"Removed {len(label_counts) - len(valid_classes)} classes with insufficient samples")
                ...
                valid_genres = [unique_genres[i] for i in valid_classes if i < len(unique_genres)]
                mapping = valid_genres
                
                logger.info(f"Converted {len(valid_genres)} string genres to integer labels")
            else:
            ...
            if 'features' in mfcc_data and 'labels' in mfcc_data:
                ...
                logger.info(f"MFCC features: {len(features_list)} samples, frame range: {min_frames}-{max_frames}")
...
def plot_ks_curves(y_true, y_probs, class_names, output_path):
    ...
    for class_idx in range(y_probs.shape[1]):
        ...
        if len(class_true) > 0:
            ...
            # Plot CDFs
            plt.plot(class_true_sorted, np.linspace(0, 1, len(class_true_sorted)), 
                    label=f'{class_names[class_idx]} (True)', alpha=0.7)
            plt.plot(class_all_sorted, np.linspace(0, 1, len(class_all_sorted)), 
                    label=f'{class_names[class_idx]} (All)', alpha=0.7, linestyle='--')
    ...
    try:
        ...
        if args.command == 'extract':
            ...
            try:
                ...
                evaluator.save_evaluation_results(results, os.path.join(args.output, 'evaluation_results.json'))
            ...
            if isinstance(result, tuple):
                ...
                logger.info(f"Final training accuracy: {training_history['val_acc'][-1]:.4f}")
