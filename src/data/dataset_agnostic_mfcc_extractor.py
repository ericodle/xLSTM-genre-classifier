...
class DatasetAgnosticMFCCExtractor:
    ...
    def extract_mfccs(self, output_path: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"Starting MFCC extraction for {dataset_metadata['name']} dataset")
        ...
        for i, (file_path, genre) in enumerate(audio_files):
            if i % 100 == 0:
                self.logger.info(f"Processing file {i+1}/{len(audio_files)}: {file_path}")
        ...
        self.logger.info(f"  - Feature shape: {len(features)} x {len(features[0])} x {len(features[0][0])}")
