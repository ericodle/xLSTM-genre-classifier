...
from core.constants import (
    ...
    DEFAULT_AUGMENTATION_FACTOR, DEFAULT_NOISE_STD, 
    DEFAULT_FEATURE_CLIP_MIN, DEFAULT_FEATURE_CLIP_MAX, DEFAULT_EPSILON
)
...
class AudioPreprocessor:
    ...
    def augment_data(self, features: np.ndarray, labels: np.ndarray, 
                    augmentation_factor: int = DEFAULT_AUGMENTATION_FACTOR) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for i in range(len(features)):
            ...
            for _ in range(augmentation_factor - 1):
                ...
                augmented_feature = np.clip(augmented_feature, DEFAULT_FEATURE_CLIP_MIN, DEFAULT_FEATURE_CLIP_MAX)
    ...
    def balance_classes(self, features: np.ndarray, labels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for label in unique_labels:
            ...
            if len(label_features) < max_count:
                ...
                indices = np.random.choice(len(label_features), additional_samples, replace=True)
    ...
    def validate_data(self, features: np.ndarray, labels: np.ndarray) -> bool:
        ...
        if len(features) != len(labels):
            self.logger.error(f"Features length ({len(features)}) != labels length ({len(labels)})")
            return False
