...
def test_recurrent_model(model, test_dataloader, device="cpu"):
    ...
    with torch.no_grad():
        for X_testbatch, y_testbatch in test_dataloader:
            ...
            h0 = torch.zeros(model.layer_dim, X_testbatch.size(0), model.hidden_dim).to(
                device
            )
            c0 = torch.zeros(model.layer_dim, X_testbatch.size(0), model.hidden_dim).to(
                device
            )
...
def plot_learning_metrics(train_loss, val_loss, train_acc, val_acc, output_directory):
    ...
    plt.savefig(
        os.path.join(output_directory, "learning_metrics.png"), bbox_inches="tight"
...
def main(mfcc_path, model_type, output_directory, initial_lr):
    ...
    sched = CyclicLR(
        opt, cosine(t_max=iterations_per_epoch * 2, eta_min=initial_lr / 100)
    )
    ...
    if model_type == "FC":
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % 5 == 0:
                print(
                    f"Epoch: {epoch:3d}. Loss: {loss.item():.4f}. Val Acc.: {vacc:2.2%}"
                )
            ...
            if vacc > best_acc:
                ...
                print(
                    f"Epoch {epoch} best model saved with val accuracy: {best_acc:2.2%}"
                )
            else:
                trials += 1
                if trials >= patience:
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            with torch.no_grad():
                for val_batch_idx, (x_val, y_val) in enumerate(val_dataloader):
                    ...
                    print(
                        f"    Batch acc: {100 * (preds == y_val).sum().item() / y_val.size(0):.2f}%"
                    )
            ...
            if epoch % 5 == 0:
                print(
                    f"Epoch: {epoch:3d}. Training Loss: {train_loss_epoch:.4f}. Val Acc.: {val_acc_epoch / 100:.2%}"
                )
            ...
            if val_acc_epoch / 100 > best_acc:
                ...
                print(
                    f"Epoch {epoch} best model saved with val accuracy: {best_acc:.2%}"
                )
            else:
                trials += 1
                if trials >= patience:
                    print(
                        f"Early stopping on epoch {epoch} due to no improvement in val accuracy."
                    )
                    break
    ...
    if model_type == "FC":
        ...
        # Test the model
        ground_truth, predicted_genres, predicted_probs, accuracy = test_ann_model(
    ...
    if model_type == "LSTM":
        ...
        # Test the model
        ground_truth, predicted_genres, predicted_probs, accuracy = (
            test_recurrent_model(model, test_dataloader, device=device)
        )
    ...
    if model_type == "Tr_FC":
        ...
        # Test the model
        ground_truth, predicted_genres, predicted_probs, accuracy = (
            test_transformer_model(model, test_dataloader)
        )
    ...
    if model_type == "Tr_LSTM":
        ...
        # Test the model
        ground_truth, predicted_genres, predicted_probs, accuracy = (
            test_transformer_model(model, test_dataloader, device=device)
        )
...
if __name__ == "__main__":
    ...
    if len(args) >= 4:
        ...
        print(
            "Please provide all required arguments: mfcc_path, model_type, output_directory, initial_lr"
        )
