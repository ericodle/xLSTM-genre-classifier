...
def test_xlstm_model(model, test_dataloader, device="cpu"):
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_dataloader:
            ...
            initial_state = [
                tuple(
                    torch.zeros(batch_size, model.xlstm.hidden_size, device=device)
                    for _ in range(4)
                )
...
def save_ann_confusion_matrix(
    ...
    arr = confusion_matrix(
        ground_truth.view(-1).detach().cpu().numpy(),
        predicted_genres.view(-1).detach().cpu().numpy(),
    )
...
def train_val_split(X, y, val_ratio):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=val_ratio, shuffle=True
    )
...
def plot_learning_metrics(train_loss, val_loss, train_acc, val_acc, output_directory):
    ...
    )  # Adjusting layout to leave space for title and legend
    fig.legend(
        loc="upper left", bbox_to_anchor=(1, 1)
    )  # Moving legend outside the plot
    plt.title("Learning Metrics", pad=20)  # Adding padding to the title
    plt.savefig(
        os.path.join(training_imgs_dir, "learning_metrics.png"), bbox_inches="tight"
    )  # Use bbox_inches='tight' to prevent cutting off
    plt.close()


def plot_comprehensive_training_metrics(
    train_loss, val_loss, train_acc, val_acc, learning_rates, output_directory
):
    """
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(training_imgs_dir, "training_overview.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    if len(train_loss) > 1:
        ...
        if len(train_loss) > 2:
            ...
            ax2.set_title(
                "Loss Curvature (Second Derivative)", fontsize=14, fontweight="bold"
            )
            ax2.set_xlabel("Epoch")
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(training_imgs_dir, "learning_analysis.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    if len(train_loss) > 5:
        ...
        ax1.set_title(
            f"Loss Variance (Window={window_size})", fontsize=14, fontweight="bold"
        )
        ax1.set_xlabel("Epoch")
    ...
    if len(train_loss) > 10:
        ...
        ax2.bar(
            ["Loss Trend"], [loss_trend], color="green" if loss_trend < 0 else "red"
        )
        ax2.set_title(
            "Recent Loss Trend (Last 10 Epochs)", fontsize=14, fontweight="bold"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(training_imgs_dir, "stability_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    with open(summary_file, "w") as f:
        ...
        if learning_rates:
            ...
            f.write(
                f"Learning Rate Reduction: {learning_rates[0]/learning_rates[-1]:.1f}x\n"
            )
...
def main(
    ...
    if class_weight_arg == "auto":
        ...
        class_weights = torch.tensor(
            [float(x) for x in class_weight_arg.split(",")],
            dtype=torch.float32,
            device=device,
        )
    ...
    if optimizer_name == "adam":
        ...
        opt = optim.SGD(
            model.parameters(), lr=initial_lr, momentum=0.9, weight_decay=1e-3
        )
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opt, mode="max", factor=0.7, patience=1, min_lr=1e-7
    )

    print(
        f"Training {model_type} model with learning rate of {initial_lr}, batch size {batch_size}, hidden size {hidden_size}, num layers {num_layers}, dropout {dropout}, optimizer {optimizer_name}."
    )
    ...
    try:
        sample_input = torch.zeros(1, 100, 16).to(
            device
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            for batch_idx, (x_batch, y_batch) in enumerate(train_dataloader):
                ...
                if epoch == 1 and batch_idx == 0:
                    ...
                    mfcc_img = (
                        x_batch[0].detach().cpu().numpy().T
                    )  # shape (n_mfcc, seq_len)
                    import matplotlib.pyplot as plt
                    import io
                ...
                if batch_idx % 20 == 0:
                    ...
                    print(
                        f"  Batch {batch_idx}/{len(train_dataloader)} - Loss: {loss.item():.4f} - Time: {batch_time:.2f}s"
                    )
            ...
            if ttotal == 0:
                print(
                    f"WARNING: Epoch {epoch} - No batches were processed (all skipped due to NaN detection)"
                )
            print(
                f"Epoch {epoch} training done. Avg Loss: {train_loss_epoch:.4f}, Acc: {train_acc_epoch:.2f}%"
            )
            ...
            writer.add_scalar(
                "Loss/Validation",
                val_loss_epoch if "val_loss_epoch" in locals() else 0,
                epoch,
            )
            writer.add_scalar("Accuracy/Train", train_acc_epoch, epoch)
            writer.add_scalar(
                "Accuracy/Validation",
                val_acc_epoch if "val_acc_epoch" in locals() else 0,
                epoch,
            )
                ...
                with open(log_file, "a") as f:
                    ...
                    for name, grads in gradient_values.items():
                        if grads:
                            ...
                            f.write(
                                f"{name:40s} | mean: {grad_mean: .5e} | std: {grad_std: .5e} | min: {grad_min: .5e} | max: {grad_max: .5e}\n"
                            )
            ...
            print(
                f"Epoch {epoch} validation done. Avg Loss: {val_loss_epoch:.4f}, Acc: {val_acc_epoch:.2f}%"
            )
            ...
            if train_acc_epoch / 100 > best_acc:
                ...
                print(
                    f"Epoch {epoch} best model saved with train accuracy: {best_acc:.2%}"
                )
            else:
                trials += 1
                if trials >= patience:
                    print(
                        f"Early stopping on epoch {epoch} due to no improvement in train accuracy."
                    )
    ...
    if model_type == "xLSTM":
        plot_learning_metrics(
            train_loss, val_loss, train_acc, val_acc, output_directory
        )
        ...
        pred_df = pd.DataFrame(
            {
                ...
                "true_label": (
                    ...
                    else np.array(ground_truth)
                ),
                "predicted_label": (
                    predicted_genres.cpu().numpy()
                    if hasattr(predicted_genres, "cpu")
                    else np.array(predicted_genres)
                ),
            }
        )
        for i, cname in enumerate(class_names):
            pred_df[f"prob_{cname}"] = (
                predicted_probs[:, i].cpu().numpy()
                if hasattr(predicted_probs, "cpu")
                else np.array(predicted_probs)[:, i]
            )
        pred_csv_path = os.path.join(
            output_directory, "predictions_vs_ground_truth.csv"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=256, help="Hidden size (default: 256)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        choices=["default", "xavier", "he"],
        help="Weight initialization (default, xavier, he)",
    )
    parser.add_argument(
        "--class_weight",
        type=str,
        default="none",
        help='Class weighting: "auto", "none", or comma-separated list (e.g. 1.0,0.5,...)',
    )
    parser.add_argument(
        "--epoch_patience",
        type=int,
        default=2,
        help="Number of epochs with no improvement to wait before early stopping (default: 2)",
    )
