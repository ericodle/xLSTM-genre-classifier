...
def config_exists(config, existing_results):
    if existing_results.empty:
        return False
    mask = (
        (existing_results["batch_size"] == config["batch_size"])
    ...
    return (
        mask.any() and not pd.isnull(existing_results.loc[mask, "test_accuracy"]).all()
    )
...
def run_one(config, run_number):
    ...
    if os.path.exists(output_dir) and os.listdir(output_dir):
        print(
            f"Warning: Output directory {output_dir} already exists and is not empty. Contents may be overwritten."
        )
...
def main():
    ...
    all_results = (
        existing_results.to_dict("records") if not existing_results.empty else []
    )
    run_number = (
        1 if existing_results.empty else (existing_results["run_number"].max() + 1)
    )
    ...
    print(
        pd.DataFrame(all_results).sort_values("test_accuracy", ascending=False).head()
    )
