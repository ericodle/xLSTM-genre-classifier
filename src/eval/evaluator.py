...
class ModelEvaluator:
    ...
    def generate_evaluation_plots(
        ...
        if "roc_auc" in results:
            self._plot_roc_curves(results, class_names, os.path.join(output_dir, "roc_curves.png"))

        # KS curves
        if "ks_stats" in results:
            self._plot_ks_curves(results, class_names, os.path.join(output_dir, "ks_curves.png"))
                ...
                for class_idx in range(y_probs.shape[1]):
                    ...
                    if len(np.unique(y_true_binary)) > 1:  # Check if class exists in test set
                        ...
                        plt.plot(
                            fpr,
                            tpr,
                            label=f"{class_name} (AUC = {roc_auc_score(y_true_binary, y_probs[:, class_idx]):.3f})",
                        )
