...
def create_mfcc_image(mfcc_data: np.ndarray, 
                     title: str = "MFCC Spectrogram",
    ...
    im = ax.imshow(normalized_data.T, 
                   aspect='auto', 
                   origin='lower', 
                   cmap='viridis',
...
def save_sample_images(features: np.ndarray, 
                      labels: np.ndarray, 
                      genre_names: List[str],
                      output_dir: str,
                      samples_per_genre: int = 3) -> None:
    ...
    for genre_idx, genre_name in enumerate(genre_names):
        ...
        for i, sample_idx in enumerate(genre_samples[:samples_per_genre]):
            ...
            fig = create_mfcc_image(
                mfcc_data, 
                title=f"{genre_name} - Sample {i+1} (Index {sample_idx})"
            )
...
def create_genre_comparison_grid(features: np.ndarray, 
                                labels: np.ndarray, 
                                genre_names: List[str],
                                output_dir: str,
                                samples_per_genre: int = 2) -> None:
...
def create_statistics_summary(features: np.ndarray, 
                             labels: np.ndarray, 
                             genre_names: List[str],
                             output_dir: str) -> None:
...
def main():
    ...
    parser.add_argument('--create-grid', action='store_true', 
                       help='Create genre comparison grid')
    parser.add_argument('--create-stats', action='store_true', 
                       help='Create statistics summary')
    ...
    # Create sample images
    save_sample_images(features, labels, genre_names, 
                      str(output_dir), args.samples_per_genre)
