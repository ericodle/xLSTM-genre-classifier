...
class TestModelTrainer:
    ...
    def sample_mfcc_data(self, temp_dir):
        ...
        for i in range(20):  # 20 samples
            ...
            data[filename] = {
                'mfcc': np.random.randn(100, 13).tolist(),  # 100 time steps, 13 MFCC coefficients
                'genre': genre
            }
...
class TestAutomaticEvaluation:
    ...
    def sample_mfcc_data(self, temp_dir):
        ...
        for i in range(20):  # 20 samples
            ...
            filename = f"{genre}/{genre}.{i:05d}.wav"
            data[filename] = {
                'mfcc': np.random.randn(100, 13).tolist(),
                'genre': genre
            }
    ...
    def test_automatic_evaluation_integration(self, mock_eval, sample_config, sample_mfcc_data, temp_dir):
        ...
        result = train_model(
            Mock(data=sample_mfcc_data, model='LSTM', output=temp_dir, epochs=1, batch_size=4),
            sample_config,
            Mock()
        )
