...
class TestGridSearchTrainer:
    ...
    def test_run_grid_search_basic(self, mock_ensure_dir, mock_trainer_class, 
                                  sample_config, sample_param_grid, temp_dir):
    ...
    @patch('training.grid_search.ModelTrainer')
    @patch('training.grid_search.ensure_directory')
    def test_run_grid_search_with_failures(self, mock_ensure_dir, mock_trainer_class,
                                         sample_config, sample_param_grid, temp_dir):
...
class TestRunGridSearchScript:
    ...
    def test_dry_run_functionality(self, temp_dir, sample_mfcc_data):
        ...
        with patch('sys.argv', ['run_grid_search.py', '--model', 'GRU', '--data', mfcc_file, '--output', temp_dir, '--dry-run']):
    ...
    def test_parameter_validation(self, temp_dir):
        ...
        with patch('sys.argv', ['run_grid_search.py', '--model', 'INVALID', '--data', 'nonexistent.json', '--output', temp_dir]):
            with pytest.raises(SystemExit):
                run_grid_search.main()
        
        # Test with nonexistent data file
        with patch('sys.argv', ['run_grid_search.py', '--model', 'GRU', '--data', 'nonexistent.json', '--output', temp_dir]):
            with pytest.raises(SystemExit):
                run_grid_search.main()
...
class TestGridSearchIntegration:
    ...
    @patch('training.grid_search.ModelTrainer')
    def test_grid_search_with_model_trainer_integration(self, mock_trainer_class, temp_dir):
