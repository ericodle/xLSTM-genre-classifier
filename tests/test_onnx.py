...
class TestONNXExport:
    ...
    def test_onnx_export_lstm(self, temp_dir):
        ...
        # Create LSTM model
        model = get_model('LSTM', input_dim=13, hidden_dim=32, num_layers=1, output_dim=10, dropout=0.1)
    ...
    def test_onnx_export_xlstm(self, temp_dir):
        ...
        # Create xLSTM model
        model = get_model('xLSTM', input_dim=13, hidden_dim=32, num_layers=1, output_dim=10, dropout=0.1)
...
class TestONNXCompatibility:
    ...
    def test_onnx_model_type_detection(self, temp_dir):
        ...
        lstm_model = get_model('LSTM', input_dim=13, hidden_dim=32, num_layers=1, output_dim=10, dropout=0.1)
...
class TestONNXIntegration:
    ...
    def test_onnx_wrapper_creation(self):
        ...
        from src.main import create_onnx_wrapper
        
        wrapper = create_onnx_wrapper(mock_session, 'RNN')
        
        assert wrapper.session == mock_session
        assert wrapper.model_type == 'RNN'
        assert hasattr(wrapper, 'eval')
        assert callable(wrapper)
    
    def test_onnx_model_type_detection(self):
        """Test ONNX model type detection function."""
        from src.main import detect_onnx_model_type
