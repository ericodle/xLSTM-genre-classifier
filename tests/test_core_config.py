...
from core.constants import (
    MFCC_COEFFICIENTS, DEFAULT_SAMPLE_RATE, DEFAULT_SONG_LENGTH,
    DEFAULT_N_FFT, DEFAULT_HOP_LENGTH, DEFAULT_SEGMENT_LENGTH,
    DEFAULT_BATCH_SIZE, DEFAULT_HIDDEN_SIZE, DEFAULT_NUM_LAYERS,
    DEFAULT_DROPOUT, DEFAULT_LEARNING_RATE, DEFAULT_NUM_EPOCHS,
    DEFAULT_EARLY_STOPPING_PATIENCE, DEFAULT_VALIDATION_SPLIT,
    DEFAULT_OPTIMIZER, DEFAULT_LOSS_FUNCTION, DEFAULT_WEIGHT_DECAY,
    DEFAULT_LR_SCHEDULER, DEFAULT_DEVICE, DEFAULT_NUM_WORKERS,
    DEFAULT_PIN_MEMORY, DEFAULT_SAVE_BEST_MODEL, DEFAULT_SAVE_CHECKPOINTS,
    DEFAULT_CHECKPOINT_INTERVAL, DEFAULT_LOG_INTERVAL, DEFAULT_RANDOM_SEED,
    DEFAULT_EARLY_STOPPING, DEFAULT_PATIENCE
)
...
class TestAudioConfig:
    ...
    def test_default_values(self):
        ...
        assert config.song_length == DEFAULT_SONG_LENGTH
        assert config.mfcc_count == MFCC_COEFFICIENTS
        assert config.n_fft == DEFAULT_N_FFT
        assert config.hop_length == DEFAULT_HOP_LENGTH
        assert config.seg_length == DEFAULT_SEGMENT_LENGTH
    ...
    def test_custom_values(self):
        ...
        assert config.song_length == 60
        assert config.mfcc_count == 20
        assert config.n_fft == DEFAULT_N_FFT  # Should use default
        assert config.hop_length == DEFAULT_HOP_LENGTH  # Should use default
        assert config.seg_length == DEFAULT_SEGMENT_LENGTH  # Should use default
...
class TestConfig:
    ...
    def test_default_initialization(self):
        ...
        assert config.audio.mfcc_count == MFCC_COEFFICIENTS
    ...
    def test_get_config_methods(self):
        ...
        assert audio_config['mfcc_count'] == MFCC_COEFFICIENTS
